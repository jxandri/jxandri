...
This applet puts them side by side and to the same scale. The left panel is
the problem; the right one is its answer. Marginal cost is drawn in both, in the
...
\figher[1.0]{panels}{The two panels at the opening settings. Left: the problem,
with marginal, average and average variable cost. Right: the firm's supply
curve in the $(q, p)$ plane. Both axes are the same in both panels, so marginal
...
\item[The firm panel] (left). Either the marginal curves or the totals,
  depending on the \ui{Firm panel} switch.
\item[The supply panel] (right). The inverse supply curve in the $(q, p)$ plane,
...
One number written three ways, and the applet draws two of them: in the left
panel as the area between $p$ and marginal cost, and in the right as the area
...
On the left the surplus measured as the integral of $p - MC$ with respect to
quantity; on the right the same number as the integral of $q$ with respect to
price.}
...
\amexp The red curve in the left panel and the cyan one in the right occupy the
...
\par\smallskip
Ask the class why the right panel has no average cost curve drawn in bold:
...
\par\smallskip
Check it with the numbers in the left panel: $q^\ast = 2.869$,
