...
const STRINGS = {
  en: {
    ...
    'fn.campusnote': 'A real place rather than a landscape: the square kilometre and a half around the Universidad de los Andes in San Carlos de Apoquindo, on the Andean foot slope at the eastern edge of Santiago — 70°30′40″W to 70°29′15″W by 33°24′S to 33°25′S, which is 2.19 km east–west by 1.84 km north–south, with 505 m of relief across it. Coordinates are km east and north of the centre of that rectangle; heights are km. Three public sources, kept deliberately apart. The <em>function</em> is the ground and nothing else: a finite cosine series fitted to the public elevation model, within about 5 m of it, and therefore infinitely differentiable — every gradient, level curve, tangent plane and geodesic here is an exact object rather than an artefact of a sampling grid. The <em>vegetation</em> is not guessed from height, as it is on every other surface in this program: ESA WorldCover 2021 records at 10 m whether each patch is tree cover, matorral scrub, dry grass, bare ground or built, and the colour and the planting both read that. The <em>buildings</em> are 1 493 real outlines from Overture Maps, with real heights where the source states one. Neither the trees nor the buildings are part of f: they stand on it. Walking into a lecture theatre does not change the altitude, a roof is not a local maximum, and the gradient at a doorway is the slope of the hillside the doorway was cut into — which is the distinction between a surface and the things on it, made concrete. One more thing is different here, and it is what makes the buildings worth drawing: this map is at <em>true scale</em>. Everywhere else the program declares whatever is on screen to be 220 m across, because the graph of x&#178;+y&#178; is not a number of metres wide; here a world unit is a real metre, so the explorer is 1.8 m, the houses are four to nineteen, and the trees stand over both.',
  ...
  es: {
    ...
    'fn.campusnote': 'Un lugar real y no un paisaje: el kilómetro y medio cuadrado en torno a la Universidad de los Andes, en San Carlos de Apoquindo, sobre el faldeo andino al oriente de Santiago — de 70°30′40″O a 70°29′15″O y de 33°24′S a 33°25′S, es decir 2,19 km de este a oeste por 1,84 km de norte a sur, con 505 m de desnivel. Las coordenadas son km al este y al norte del centro de ese rectángulo; las alturas, km. Tres fuentes públicas, mantenidas a propósito separadas. La <em>función</em> es el suelo y nada más: una serie finita de cosenos ajustada al modelo público de elevación, a unos 5 m de él, y por tanto infinitamente diferenciable — cada gradiente, curva de nivel, plano tangente y geodésica de aquí es un objeto exacto y no un artefacto de la malla de muestreo. La <em>vegetación</em> no se infiere de la altura, como en todas las demás superficies de este programa: ESA WorldCover 2021 registra cada 10 m si el trozo de suelo es arbolado, matorral, pasto seco, suelo desnudo o construido, y tanto el color como la plantación leen eso. Los <em>edificios</em> son 1 493 huellas reales de Overture Maps, con alturas reales donde la fuente las declara. Ni los árboles ni los edificios forman parte de f: se apoyan sobre ella. Entrar a una sala no cambia la altitud, un techo no es un máximo local, y el gradiente en una puerta es la pendiente del cerro en que se excavó esa puerta — que es, hecha concreta, la distinción entre una superficie y las cosas que están sobre ella. Hay algo más distinto aquí, y es lo que hace que valga la pena dibujar los edificios: este mapa está a <em>escala verdadera</em>. En el resto del programa se declara que lo que está en pantalla mide 220 m de ancho, porque la gráfica de x&#178;+y&#178; no mide metros; aquí una unidad del mundo es un metro real, así que el explorador mide 1,8 m, las casas de cuatro a diecinueve, y los árboles se alzan sobre ambos.',
