...
const STRINGS = {
  en: {
    ...
    'fn.eliasnote': 'Real elevation data: the 40 km around Mount Saint Elias (Was’eitushaa, Boundary Peak 186, 5 489 m), from the USGS and Canadian national elevation models, sampled every 150 m and carried inside this file. Coordinates are km east and north of the summit; heights are km, so the HUD reads in real kilometres. The feasible set is Alaska: locally the 1903 boundary is a straight line — a linear constraint, exactly like a budget line — and it passes about 470 m north-west of the summit, leaving the peak in Canada. So the highest point of the United States side cannot be the summit: press O and the optimiser finds it, pinned to the line where the level curve is tangent to the frontier — a Lagrange condition you can walk to. The relief is real — 5.4 km over a 40 km window — so the mountain is honestly gentle at true scale; stretch the z-axis dial to exaggerate it the way an atlas does. The window does not follow the explorer here, because the survey has edges.',
